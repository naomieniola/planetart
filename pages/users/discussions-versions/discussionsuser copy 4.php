...
<form id="addCommentForm">
    <!-- Display user's comment with full name above the textarea -->
    <div id="userCommentContainer" class="mb-2"></div>
    
    <textarea class="form-control mb-2" name="commentInput" id="commentInput" placeholder="Type your comment here"></textarea>
    <button type="button" class="btn btn-primary btn-add-comment" name="addCommentButton" id="addCommentButton">Add Comment</button>
</form>
...
<script>
    $(document).ready(function () {
        $("#addCommentButton").click(function () {
            ...
            var userFullname = "<?php echo htmlspecialchars($_SESSION["user_fullname"]); ?>";

            // Display user's comment with full name above the textarea
            var userComment = '<p><strong>' + userFullname + ':</strong> ' + commentInput + '</p>';
            $("#userCommentContainer").html(userComment);
            ...
            $.ajax({
                type: "POST",
                url: "your_php_script.php", // Replace with the actual PHP script handling the comment submission
                data: {
